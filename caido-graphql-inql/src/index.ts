import { registerPlugin, PluginContext, ProxyEvent } from "@caido/sdk";
import { fetchSchema, SchemaSummary } from "./schema";
import { probeOperations } from "./discovery";

interface TargetRecord {
  endpoint: string;
  headers: Record<string, string>;
  lastRequest?: ProxyEvent["request"];
  lastResponse?: ProxyEvent["response"];
  schema?: SchemaSummary;
  schemaErrors?: string[];
  discoveries?: Awaited<ReturnType<typeof probeOperations>>;
}

const targets = new Map<string, TargetRecord>();

registerPlugin(async (context: PluginContext) => {
  const panel = context.ui.createPanel({ title: "GraphQL INQL" });

  context.events.onRequestFinished(async (event) => {
    if (!looksLikeGraphQL(event)) return;
    const origin = extractOrigin(event.request.url);
    const record: TargetRecord = targets.get(origin) ?? {
  ...
  (globalThis as any).caidoGraphQLPlugin = createUiBridge(context, panel);
  render(panel);
});

function createUiBridge(context: PluginContext, panel: any) {
  return {
    async introspect(origin: string) {
    ...
    async discover(origin: string) {
      const record = targets.get(origin);
      if (!record) return;
      record.discoveries = await probeOperations(
        context.http,
        record.endpoint,
        record.headers
      );
    ...
    async replay(origin: string, textareaId: string) {
      const record = targets.get(origin);
      if (!record) return;
...
function renderTarget(origin: string, record: TargetRecord): string {
...
function renderSchema(schema: SchemaSummary): string {
...
function renderDiscoveries(result: Awaited<ReturnType<typeof probeOperations>>): string {
  const rows = result.discovered
    .map(
      (entry) => `<li class="${entry.success ? "success" : "muted"}">${entry.operation}: ${entry.field}</li>`
    )
    .join("");
...
function renderScripts(): string {
  return `
    <script>
      (function(){
        const plugin = (window as any).caidoGraphQLPlugin;
...
function looksLikeGraphQL(event: ProxyEvent): boolean {
  const contentType = (event.request.headers["content-type"] || "").toLowerCase();
  if (contentType.includes("graphql")) return true;
  if (contentType.includes("application/json") && event.request.body?.includes("query")) return true;
  if (event.request.url.includes("graphql")) return true;
