...
export function buildExampleQuery(schema: SchemaSummary, fieldName: string): string {
  const field = schema.queries.find((q) => q.name === fieldName);
  if (!field) return "# Field not found in schema";
...
export function buildExampleMutation(schema: SchemaSummary, fieldName: string): string {
  const field = schema.mutations.find((m) => m.name === fieldName);
  if (!field) return "# Mutation not found in schema";
  const args = field.args
    .map((arg, index) => `$var${index}: ${arg.split(":")[1].trim()}`)
    .join(", ");
  const argValues = field.args
